...
#define ST7735_SPI_CLOCK_HZ     (10 * 1000 * 1000)
...
public:
    ...
    // Declare all structs upfront to avoid goto bypassing initialization
    esp_err_t ret;
    ...
    // --- Cleanup on failure - unwind in reverse init order ---
    err_panel:
    ...
    uint16_t* buf = const_cast<uint16_t*>(framebuffer);
